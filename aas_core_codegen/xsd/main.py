...
# prescribes, and not in the target namespace of this schema, so each of their
# declarations is ``form="unqualified"``.
...
def _xml_rpc_element(attributes: Mapping[str, str]) -> ET.Element:
    """
    Make the declaration of an element of the XML-RPC subset.

    Unlike every element which the meta-model itself prescribes, an XML-RPC
    element resides in no namespace at all, as the XML-RPC specification
    prescribes. The schema declares its target namespace as the default one,
    so each of these declarations has to opt out of it.
    """
    return ET.Element("xs:element", {**attributes, "form": "unqualified"})


def _xml_rpc_value_element(
    attributes: Optional[Mapping[str, str]] = None,
) -> ET.Element:
    """
    Make the declaration of a nested XML-RPC ``<value>`` element.

    A member's value, and an item of an array, is itself an arbitrary
    ``JSONValue``, so this closes the recursion. It has to be a local
    declaration and no reference to a global one: a global element is always
    qualified, and these elements reside in no namespace.
    """
    return _xml_rpc_element(
        {
            "name": _JSON_VALUE_ELEMENT_NAME,
            "type": _JSON_VALUE_TYPE_NAME,
...
def _generate_json_type_definitions() -> List[ET.Element]:
    ...
    ``jsonValue_t`` is genuinely self-referential -- a struct member's value,
    or an array item, is itself an arbitrary ``JSONValue`` -- and the local
    ``<value>`` declarations close that recursion.
    ``jsonArray_t``/``jsonStruct_t`` are ordinary named complex types, shared
    the same way every other named type in this backend is shared (through
    a ``type="..."`` attribute), not because they are self-referential.

    The root element is to be *extended* with the resulting list.
    """
    xs_value_choice = ET.Element("xs:choice")
    xs_value_choice.append(_xml_rpc_element({"name": "boolean", "type": "xs:boolean"}))
    xs_value_choice.append(_xml_rpc_element({"name": "double", "type": "xs:double"}))
    xs_value_choice.append(_xml_rpc_element({"name": "string", "type": "xs:string"}))
    xs_value_choice.append(
        _xml_rpc_element({"name": "array", "type": _JSON_ARRAY_TYPE_NAME})
    )
    xs_value_choice.append(
        _xml_rpc_element({"name": "struct", "type": _JSON_STRUCT_TYPE_NAME})
    )
    ...
    xs_data = _xml_rpc_element({"name": "data"})
    ...
    xs_data_sequence.append(
        _xml_rpc_value_element({"minOccurs": "0", "maxOccurs": "unbounded"})
    )
    ...
    xs_member = _xml_rpc_element(
        {"name": "member", "minOccurs": "0", "maxOccurs": "unbounded"}
    )
    ...
    xs_member_sequence.append(_xml_rpc_element({"name": "name", "type": "xs:string"}))
    xs_member_sequence.append(_xml_rpc_value_element())
...
def _value_to_type_element_or_type_identifier(
    ...
    if primitive_type is not None:
        ...
        if isinstance(type_annotation, intermediate.PrimitiveTypeAnnotation):
            ...
            xs_name = _xml_rpc_element({"name": "name"})
