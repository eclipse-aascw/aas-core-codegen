...
{II}// a JSON-able value, even though ``nlohmann::json`` will happily hold
{II}// one if it was constructed programmatically. This mirrors what the
{II}// jsonization refuses to serialize.
{II}if (value.is_number_float() && !std::isfinite(value.get<double>())) {{
{III}std::wstring message = common::Concat(
