...
{I}/**
{I} * \\brief XPath to the erroneous value, relative to the element at which
{I} * the de-serialization started
{I} *
{I} * The caller prepends the elements enclosing that element, so that
{I} * the resulting XPath descends from the XML document to the value.
{I} */
...
{I}/**
{I} * \\brief Path to the value that caused the error, within the value
{I} * that we were given to serialize
{I} *
{I} * Unlike the de-serialization, which reports where in the document
{I} * the error occurred, we report where in the value it occurred -- the
{I} * document has not been written at all.
{I} */
{I}iteration::Path path;

{I}explicit SerializationError(std::wstring a_cause);
{I}SerializationError(std::wstring a_cause, iteration::Path a_path);
}};  // struct SerializationError"""
        ),
    ]
...
{I}// NOTE (mristin):
{I}// A boolean is a number-like value, so the whitespace around it is
{I}// collapsed before it is matched, just as `whiteSpace="collapse"`
{I}// prescribes for every atomic XSD type but a string. Only a <string>
{I}// keeps its whitespace.
...
{I}// NOTE (mristin):
{I}// See the note in ``DeserializeBooleanBodyFrom`` on why the whitespace
{I}// is collapsed here.
{I}const std::string text(
{II}xml_common::CollapseWhitespace(
{III}static_cast<  // NOLINT(cppcoreguidelines-pro-type-static-cast-downcast)
...
{I}// NOTE (mristin):
{I}// The lexical form has to be checked before the text is parsed.
{I}// std::strtod reads far more than we admit here: a hexadecimal
{I}// significand, so "0x10" would come out as 16; a trailing remainder, so
{I}// "1.0abc" would come out as 1; and the spellings "inf", "infinity",
{I}// "nan" and "NAN".
{I}//
{I}// Mind that the numeral excludes "INF", "-INF" and "NaN" on purpose,
{I}// unlike xs:double, which names all three. A <double> carries a JSON
...
{I}if (!xml_common::MatchesXsDoubleNumeral(text)) {{
{II}return std::make_pair(
{III}common::nullopt,
{III}common::make_optional<DeserializationError>(
{IIII}common::Concat(
...
{I}// NOTE (mristin):
{I}// std::strtod reports the range through errno, which we deliberately
{I}// leave alone: a literal too large gives an infinity and one too small
{I}// gives a zero, and only the infinity is a problem here. The numeral has
{I}// already been matched, so strtod consumes all of it.
{I}const double deserialized = std::strtod(text.c_str(), nullptr);

{I}if (!std::isfinite(deserialized)) {{
{II}return std::make_pair(
{III}common::nullopt,
{III}common::make_optional<DeserializationError>(
{IIII}common::Concat(
{IIIII}L"Expected a number representable as a JSON-able value as "
{IIIII}L"the text of a <double> element, but got a value which "
{IIIII}L"rounds to an infinity: ",
{IIIII}common::Utf8ToWstring(text)
{IIII})
{III})
{II});
...
{II}case xml_common::NodeKind::StopInNoNamespace:
{III}// NOTE (mristin):
...
{I}if (reader.node().kind() != xml_common::NodeKind::StartInNoNamespace) {{
{II}return std::make_pair(
{III}common::nullopt,
{III}common::make_optional<DeserializationError>(
{IIII}common::Concat(
...
{II}// NOTE (mristin):
{II}// We are inside the element which discriminates the value, so we grow
{II}// the path with it. The functions which de-serialize an array or
{II}// a struct body without such a wrapping element are called directly,
{II}// bypassing us, and hence correctly report no wrapping element either.
{II}error->path.segments.emplace_front(
{III}common::make_unique<xml_path::ElementSegment>(
{IIII}common::Utf8ToWstring(discriminator)
{III})
{II});
...
{I}if (!xml_common::IsStopNodeWithNameInNoNamespace(reader.node(), discriminator)) {{
{II}return std::make_pair(
{III}common::nullopt,
{III}common::make_optional<DeserializationError>(
{IIII}common::Concat(
...
{I}if (
{II}reader.node().kind() != xml_common::NodeKind::StartInNoNamespace
{II}|| static_cast<  // NOLINT(cppcoreguidelines-pro-type-static-cast-downcast)
{III}const xml_common::StartNode&
...
{IIII}common::make_unique<xml_path::IndexSegment>(index)
{III});
{III}error->path.segments.emplace_front(
{IIII}common::make_unique<xml_path::ElementSegment>(L"data")
{III});
{III}return std::make_pair(common::nullopt, std::move(error));
{II}}}

{II}if (!xml_common::IsStopNodeWithNameInNoNamespace(reader.node(), "value")) {{
{III}return std::make_pair(
{IIII}common::nullopt,
{IIII}common::make_optional<DeserializationError>(
{IIIII}common::Concat(
...
{I}if (!xml_common::IsStopNodeWithNameInNoNamespace(reader.node(), "data")) {{
{II}return std::make_pair(
{III}common::nullopt,
{III}common::make_optional<DeserializationError>(
{IIII}common::Concat(
...
{II}if (
{III}reader.node().kind() != xml_common::NodeKind::StartInNoNamespace
{III}|| static_cast<  // NOLINT(cppcoreguidelines-pro-type-static-cast-downcast)
...
{III}case xml_common::NodeKind::StopInNoNamespace:
{IIII}key = std::string();
{IIII}break;
...
{II}if (!xml_common::IsStopNodeWithNameInNoNamespace(reader.node(), "name")) {{
{III}return std::make_pair(
{IIII}common::nullopt,
{IIII}common::make_optional<DeserializationError>(
{IIIII}common::Concat(
...
{II}if (error.has_value()) {{
{III}// <member><name/><value/></member>
{III}error->path.segments.emplace_front(
{IIII}common::make_unique<xml_path::ElementSegment>(L"value")
{III});
{III}error->path.segments.emplace_front(
{IIII}common::make_unique<xml_path::MemberSegment>(
...
{II}if (!xml_common::IsStopNodeWithNameInNoNamespace(reader.node(), "member")) {{
{III}return std::make_pair(
{IIII}common::nullopt,
{IIII}common::make_optional<DeserializationError>(
{IIIII}common::Concat(
...
{II}// -- what ``nlohmann::json``'s own ``operator[]`` assignment would do
{II}// -- would silently accept a document which says two different things
{II}// about the same key.
{II}if (result.contains(key)) {{
{III}return std::make_pair(
{IIII}common::nullopt,
{IIII}common::make_optional<DeserializationError>(
{IIIII}common::Concat(
{IIIIII}L"The member ",
{IIIIII}common::Utf8ToWstring(key),
{IIIIII}L" occurred more than once"
{IIIII})
{IIII})
{III});
{II}}}
...
{I}if (!xml_common::IsStopNodeWithNameInNoNamespace(reader.node(), "value")) {{
{II}return common::make_unexpected(
{III}DeserializationError(
{IIII}common::Concat(
...
{III}"boolean", undeclare_namespace
{II});
{II}// NOTE (mristin):
...
{II}// a JSON-able value. ``SelfClosingWriter::SerializeDouble`` would
{II}// happily write "INF" or "NaN" -- the xs:double spellings -- which
{II}// ``DeserializeDoubleBodyFrom`` deliberately refuses to read back.
{II}if (!std::isfinite(number)) {{
{III}return common::make_optional<SerializationError>(
{IIII}common::Concat(
{IIIII}L"Expected a JSON-able value, but got the number ",
{IIIII}std::to_wstring(number),
...
{II}return common::make_optional<SerializationError>(
{III}common::Concat(
{IIII}L"Expected a JSON array, but got a value of type: ",
{IIII}common::Utf8ToWstring(value.type_name())
{III})
{II});
{I}}}

{I}writer.StartElementInNoNamespace(
{II}"data", undeclare_namespace
{I});
...
{III}writer, item, false
{II});
{II}if (error.has_value()) {{
{III}error->path.segments.emplace_front(
{IIII}common::make_unique<iteration::IndexSegment>(index)
{III});
{III}return error;
{II}}}
...
{II}return common::make_optional<SerializationError>(
{III}common::Concat(
{IIII}L"Expected a JSON object, but got a value of type: ",
{IIII}common::Utf8ToWstring(value.type_name())
{III})
{II});
{I}}}
...
{II}writer.StartElementInNoNamespace(
{III}"member", undeclare_namespace
{II});
...
{I}error = SerializeValueBodyTo(writer, value, false);
{I}if (error.has_value()) {{
{II}return error;
{I}}}
