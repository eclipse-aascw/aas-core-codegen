...
def _verify_only_simple_type_patterns(symbol_table: SymbolTable) -> List[Error]:
    ...
    * Tuples of optionals are unexpected;
    ...
    * The key of a ``JSONObject`` must be ``str`` or a class (transitively)
    ...
    Note that a list of lists is *not* supported -- the code generators report
    an error where they would otherwise have to descend into a nested list.
    A JSON-able value is a different matter: an array of arrays is a perfectly
    good ``JSONValue``, but its shape is a run-time property which we can not
    know at the code generation time, so there is nothing for us to verify
    here.
    """
    errors = []  # type: List[Error]
    for cls in symbol_table.classes:
