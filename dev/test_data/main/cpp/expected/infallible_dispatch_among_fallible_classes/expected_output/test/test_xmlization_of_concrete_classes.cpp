...
TEST_CASE("Test the de-serialization failure on a duplicate property") {
  ...
  const std::size_t start(
    original.find("<someText>")
  );

  std::string property;
  if (start != std::string::npos) {
    const std::size_t end(
      original.find("</someText>", start)
    );
  ...
  const std::size_t insertion_index(
    original.rfind("</abstractDescendantWithoutNumbers>")
  );

  INFO(aas::common::Concat("Looking for </abstractDescendantWithoutNumbers> in ", path.string()))
