...
TEST_CASE("Test Descent over an AbstractDescendantWithoutNumbers") {
...
TEST_CASE("Test DescentOnce over an AbstractDescendantWithoutNumbers") {
  const std::shared_ptr<
    aas::types::IAbstractDescendantWithoutNumbers
  > instance(
    test::common::examples::LoadMaxAbstractDescendantWithoutNumbers()
  );

  const std::filesystem::path trace_path(
    DetermineDescentOnceDir()
      / "MaxAbstractDescendantWithoutNumbers.trace"
  );

  AssertOrRerecordDescentOnce(instance, trace_path);
}

TEST_CASE("Test Descent over an ParentWithoutNumbers") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineXmlDir()
        / "Expected"
...
TEST_CASE("Test DescentOnce over an ParentWithoutNumbers") {
  const std::shared_ptr<
    aas::types::IParentWithoutNumbers
  > instance(
    test::common::examples::LoadMaxParentWithoutNumbers()
  );

  const std::filesystem::path trace_path(
    DetermineDescentOnceDir()
      / "MaxParentWithoutNumbers.trace"
  );

  AssertOrRerecordDescentOnce(instance, trace_path);
}

TEST_CASE("Test Descent over an ChildWithoutNumbers") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineXmlDir()
        / "Expected"
...
TEST_CASE("Test DescentOnce over an ChildWithoutNumbers") {
  const std::shared_ptr<
    aas::types::IChildWithoutNumbers
  > instance(
    test::common::examples::LoadMaxChildWithoutNumbers()
  );

  const std::filesystem::path trace_path(
    DetermineDescentOnceDir()
      / "MaxChildWithoutNumbers.trace"
  );

  AssertOrRerecordDescentOnce(instance, trace_path);
}

TEST_CASE("Test Descent over an WithNumber") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineXmlDir()
        / "Expected"
...
TEST_CASE("Test DescentOnce over an WithNumber") {
  const std::shared_ptr<
    aas::types::IWithNumber
  > instance(
    test::common::examples::LoadMaxWithNumber()
  );
