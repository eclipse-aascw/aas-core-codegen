...
TEST_CASE("Test the de-serialization failure on a duplicate property") {
  ...
  const std::size_t start(
    original.find("<uniqueToFirst>")
  );

  std::string property;
  if (start != std::string::npos) {
    const std::size_t end(
      original.find("</uniqueToFirst>", start)
    );
    property = original.substr(start, end + 16 - start);
  } else {
  ...
  const std::size_t insertion_index(
    original.rfind("</structuralFirst>")
  );

  INFO(aas::common::Concat("Looking for </structuralFirst> in ", path.string()))
