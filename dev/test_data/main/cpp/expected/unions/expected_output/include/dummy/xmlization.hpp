#ifndef DUMMY_XMLIZATION_GUARD_
#define DUMMY_XMLIZATION_GUARD_

// This code has been automatically generated by aas-core-codegen.
// Do NOT edit or append.

#include "dummy/common.hpp"
#include "dummy/iteration.hpp"
#include "dummy/types.hpp"
#include "dummy/xml_path.hpp"

#pragma warning(push, 0)
#include <deque>
#include <memory>
#include <string>
#pragma warning(pop)

namespace dummy {

/**
 * \defgroup xmlization De/serialize instances from and to XML.
 * @{
 */
namespace xmlization {

/**
 * Specify the expected XML namespace of all the XML elements.
 */
extern const std::string kNamespace;

// region De-serialization

/**
 * Represent a de-serialization error.
 */
struct DeserializationError {
  /**
   * Human-readable description of the error
   */
  std::wstring cause;

  /**
   * Path to the erroneous value
   */
  xml_path::Path path;

  explicit DeserializationError(std::wstring a_cause);
  DeserializationError(std::wstring a_cause, xml_path::Path a_path);
};  // struct DeserializationError

struct ReadingOptions {
  /**
   * No XML attributes are expected in XML elements.
   * Usually, attributes are considered errors and reported as such. However,
   * some implementations add their own custom attributes, and we sometimes
   * still want to parse such XML. If `additional_attributes` is set,
   * the unexpected XML attributes will be ignored during parsing, and not
   * reported.
   */
  bool additional_attributes = false;

  /**
   * Size of the chunk to be read from the input stream and passed to
   * the XML parser.
   */
  size_t buffer_size = 1024;
};  // struct ReadingOptions

/**
 * Deserialize the instance from an XML read from the stream \p is.
 *
 * \param is stream of ASCII, ISO-8859-1 or UTF-8-encoded characters to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed instance, or an error if any
 */
common::expected<
  std::shared_ptr<types::IClass>,
  DeserializationError
> From(
  std::istream& is,
  const ReadingOptions& options = {}
);

/**
 * Deserialize an instance of types::IStructuralFirst from an XML
 * read from the stream \p is.
 *
 * \param is stream to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed types::IStructuralFirst, or an error if any
 */
common::expected<
  std::shared_ptr<types::IStructuralFirst>,
  DeserializationError
> StructuralFirstFrom(
  std::istream& is,
  const ReadingOptions& options = {}
);

/**
 * Deserialize an instance of types::IStructuralSecond from an XML
 * read from the stream \p is.
 *
 * \param is stream to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed types::IStructuralSecond, or an error if any
 */
common::expected<
  std::shared_ptr<types::IStructuralSecond>,
  DeserializationError
> StructuralSecondFrom(
  std::istream& is,
  const ReadingOptions& options = {}
);

/**
 * Deserialize an instance of types::IMixedAbstractMember from an XML
 * read from the stream \p is.
 *
 * \param is stream to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed types::IMixedAbstractMember, or an error if any
 */
common::expected<
  std::shared_ptr<types::IMixedAbstractMember>,
  DeserializationError
> MixedAbstractMemberFrom(
  std::istream& is,
  const ReadingOptions& options = {}
);

/**
 * Deserialize an instance of types::IMixedAbstractDescendantOne from an XML
 * read from the stream \p is.
 *
 * \param is stream to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed types::IMixedAbstractDescendantOne, or an error if any
 */
common::expected<
  std::shared_ptr<types::IMixedAbstractDescendantOne>,
  DeserializationError
> MixedAbstractDescendantOneFrom(
  std::istream& is,
  const ReadingOptions& options = {}
);

/**
 * Deserialize an instance of types::IMixedAbstractDescendantTwo from an XML
 * read from the stream \p is.
 *
 * \param is stream to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed types::IMixedAbstractDescendantTwo, or an error if any
 */
common::expected<
  std::shared_ptr<types::IMixedAbstractDescendantTwo>,
  DeserializationError
> MixedAbstractDescendantTwoFrom(
  std::istream& is,
  const ReadingOptions& options = {}
);

/**
 * Deserialize an instance of types::IMixedConcreteWithDescendants from an XML
 * read from the stream \p is.
 *
 * \param is stream to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed types::IMixedConcreteWithDescendants, or an error if any
 */
common::expected<
  std::shared_ptr<types::IMixedConcreteWithDescendants>,
  DeserializationError
> MixedConcreteWithDescendantsFrom(
  std::istream& is,
  const ReadingOptions& options = {}
);

/**
 * Deserialize an instance of types::IMixedConcreteWithDescendantsChild from an XML
 * read from the stream \p is.
 *
 * \param is stream to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed types::IMixedConcreteWithDescendantsChild, or an error if any
 */
common::expected<
  std::shared_ptr<types::IMixedConcreteWithDescendantsChild>,
  DeserializationError
> MixedConcreteWithDescendantsChildFrom(
  std::istream& is,
  const ReadingOptions& options = {}
);

/**
 * Deserialize an instance of types::IMixedConcreteLeaf from an XML
 * read from the stream \p is.
 *
 * \param is stream to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed types::IMixedConcreteLeaf, or an error if any
 */
common::expected<
  std::shared_ptr<types::IMixedConcreteLeaf>,
  DeserializationError
> MixedConcreteLeafFrom(
  std::istream& is,
  const ReadingOptions& options = {}
);

/**
 * Deserialize an instance of types::IModelTypedFirst from an XML
 * read from the stream \p is.
 *
 * \param is stream to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed types::IModelTypedFirst, or an error if any
 */
common::expected<
  std::shared_ptr<types::IModelTypedFirst>,
  DeserializationError
> ModelTypedFirstFrom(
  std::istream& is,
  const ReadingOptions& options = {}
);

/**
 * Deserialize an instance of types::IModelTypedSecond from an XML
 * read from the stream \p is.
 *
 * \param is stream to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed types::IModelTypedSecond, or an error if any
 */
common::expected<
  std::shared_ptr<types::IModelTypedSecond>,
  DeserializationError
> ModelTypedSecondFrom(
  std::istream& is,
  const ReadingOptions& options = {}
);

/**
 * Deserialize an instance of types::ISomething from an XML
 * read from the stream \p is.
 *
 * \param is stream to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed types::ISomething, or an error if any
 */
common::expected<
  std::shared_ptr<types::ISomething>,
  DeserializationError
> SomethingFrom(
  std::istream& is,
  const ReadingOptions& options = {}
);

/**
 * Deserialize an instance of types::StructuralUnion from an XML
 * read from the stream \p is.
 *
 * \param is stream to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed types::StructuralUnion, or an error if any
 */
common::expected<
  types::StructuralUnion,
  DeserializationError
> StructuralUnionFrom(
  std::istream& is,
  const ReadingOptions& options = {}
);

/**
 * Deserialize an instance of types::MixedUnion from an XML
 * read from the stream \p is.
 *
 * \param is stream to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed types::MixedUnion, or an error if any
 */
common::expected<
  types::MixedUnion,
  DeserializationError
> MixedUnionFrom(
  std::istream& is,
  const ReadingOptions& options = {}
);

/**
 * Deserialize an instance of types::ModelTypedUnion from an XML
 * read from the stream \p is.
 *
 * \param is stream to read XML from
 * \param options reading options to be tweaked for special cases. The defaults should
 * work in most cases.
 * \return the parsed types::ModelTypedUnion, or an error if any
 */
common::expected<
  types::ModelTypedUnion,
  DeserializationError
> ModelTypedUnionFrom(
  std::istream& is,
  const ReadingOptions& options = {}
);

// endregion Deserialization

// region Serialization

/**
 * Represent an error in the serialization of an instance to XML.
 */
class SerializationException : public std::exception {
 public:
  SerializationException(
    std::wstring cause
  );

  SerializationException(
    std::wstring cause,
    iteration::Path path
  );

  const char* what() const noexcept override;

  const std::wstring& cause() const noexcept;
  const iteration::Path& path() const noexcept;

  ~SerializationException() noexcept override = default;

 private:
  const std::wstring cause_;
  const iteration::Path path_;
  const std::string msg_;
};  // class SerializationException

/**
 * \brief Customize how instances should be serialized to XML.
 *
 * We selected the defaults so that they can be used when you serialize to
 * a file.
 *
 * Usually, you want to write the namespace at the root element, and no
 * prefixes are written in the XML names. However, if you are embedding
 * the XML in a larger XML structure, you specify the namespace
 * aliases and then use them as XML name prefixes. The prefix usually ends
 * with a full colon (`:`).
 *
 * We can not imagine in what situation you would want to write both
 * the namespace <em>and</em> the prefix. Nevertheless, we allow for that
 * possibility and do not throw any exception if you specify the both.
 */
struct WritingOptions {
  /**
   * If set, the XML declaration is written at the beginning.
   */
  bool write_declaration = true;

  /**
   * If set, the root XML element is written with the XML namespace
   * set as the XML attribute `xmlns`.
   */
  bool write_namespace = true;

  /**
   * The prefix is prepended to the name of each XML element.
   */
   std::string prefix = "";
};  // struct WritingOptions

/**
 * \brief Serialize \p that instance to XML.
 *
 * \param that instance to be serialized
 * \param options  to be tweaked for special cases. The defaults should
 * work in most cases.
 * \param os The UTF8-encoded output stream where XML will be written
 * \throw \ref SerializationException if \p that instance could not be serialized
 */
void Serialize(
  const types::IClass& that,
  const WritingOptions& options,
  std::ostream& os
);

// endregion Serialization

}  // namespace xmlization
/**@}*/

}  // namespace dummy

// This code has been automatically generated by aas-core-codegen.
// Do NOT edit or append.

#endif  // DUMMY_XMLIZATION_GUARD_
