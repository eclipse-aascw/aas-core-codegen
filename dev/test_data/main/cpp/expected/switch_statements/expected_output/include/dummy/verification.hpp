...
/// \brief Check the switch with a case per form of labels and a default.
bool SwitchOnEnumWithDefault(
  types::Kind kind
);

/// \brief Check the switch with the labels given as a tuple.
bool SwitchOnEnumWithLabelsInTuple(
  types::Kind kind
);

/// \brief Check the switch with the branches which pass and complete normally.
bool SwitchOnEnumWithPass(
  types::Kind kind
);

/// \brief Check the variables defined in the cases of the switch.
bool SwitchOnEnumWithVariablesInCases(
  types::Kind kind,
  const std::wstring& text
);

/// \brief Check the switch on a string.
bool SwitchOnStr(
  const std::wstring& text
);
...
/// \brief Check the switch on an integer.
bool SwitchOnInt(
  int64_t number
);
