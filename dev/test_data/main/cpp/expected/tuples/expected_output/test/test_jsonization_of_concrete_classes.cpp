...
TEST_CASE(
  ...
  "at .serial_number of AnotherItem"
) {
  for (
    const int64_t value
    : {
    ...
    std::shared_ptr<aas::types::IAnotherItem> instance(
      MustDeserializeTheFirstExpected<aas::types::IAnotherItem>(
        "AnotherItem",
        aas::jsonization::AnotherItemFrom
      )
    );

    instance->set_serial_number(value);
