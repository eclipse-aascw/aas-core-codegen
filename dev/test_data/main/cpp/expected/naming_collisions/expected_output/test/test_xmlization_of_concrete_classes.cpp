...
TEST_CASE("Test the round-trip of an expected Instance") {
...
TEST_CASE("Test the de-serialization failure on an unexpected Instance") {
  for (
...
TEST_CASE("Test the round-trip of an expected Pointer") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineXmlDir()
        / "Expected"
...
TEST_CASE("Test the de-serialization failure on an unexpected Pointer") {
  for (
    const std::filesystem::path& causeDir
    : test::common::ListSubdirectories(
      DetermineXmlDir()
...
TEST_CASE("Test the round-trip of an expected JsonObjectOfName") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineXmlDir()
        / "Expected"
...
TEST_CASE("Test the de-serialization failure on an unexpected JsonObjectOfName") {
  for (
    const std::filesystem::path& causeDir
    : test::common::ListSubdirectories(
      DetermineXmlDir()
...
TEST_CASE("Test the round-trip of an expected ListOf") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineXmlDir()
        / "Expected"
...
TEST_CASE("Test the de-serialization failure on an unexpected ListOf") {
  for (
    const std::filesystem::path& causeDir
    : test::common::ListSubdirectories(
      DetermineXmlDir()
...
TEST_CASE("Test the de-serialization failure on a duplicate property") {
  ...
  const std::size_t insertion_index(
    original.rfind("</instance>")
  );

  INFO(aas::common::Concat("Looking for </instance> in ", path.string()))
