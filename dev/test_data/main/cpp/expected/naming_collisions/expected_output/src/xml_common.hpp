...
enum class NodeKind : std::uint32_t {
  ...
  Bof = 0,
  // Nodes of the kind `Start` and `Stop` represent an element which resides in
  // \ref kNamespace.
  Start = 1,
  Stop = 2,
  Text = 3,
  ...
  Error = 5,
  // Nodes of the kind `StartInNoNamespace` and `StopInNoNamespace` represent
  // an element which resides in no namespace at all. Only the XML-RPC subset,
  // over which a JSON-able value is de/serialized, is written like that.
  StartInNoNamespace = 6,
  StopInNoNamespace = 7
};  // enum class NodeKind
...
class StartNode : public INode {
 public:
  StartNode(
    std::string a_name,
    bool an_in_namespace
  ) :
    name(std::move(a_name)),
    in_namespace(an_in_namespace) {
    // Intentionally empty.
  }
  ...
  NodeKind kind() const override {
    return in_namespace
      ? NodeKind::Start
      : NodeKind::StartInNoNamespace;
  }

  /**
   * Name of the start element, stripped of the XML namespace
   */
  const std::string name;

  /**
   * True if the element resides in \ref kNamespace, and false if it resides
   * in no namespace at all
   */
  const bool in_namespace;
...
class StopNode : public INode {
 public:
  ...
  NodeKind kind() const override {
    return in_namespace
      ? NodeKind::Stop
      : NodeKind::StopInNoNamespace;
  }

  /**
   * Name of the stop element, stripped of the XML namespace
   */
  const std::string name;
...
class Reader {
 public:
   ...
   *
   * Every element is expected to reside either in \ref kNamespace or in no
   * namespace at all -- the latter is how the XML-RPC subset, over which
   * a JSON-able value is de/serialized, is written. An element in any other
   * namespace is reported as an error. Which of the two an element is expected
   * to reside in depends on the grammar, so the node says which one it was, and
   * the caller decides.
   */
  Reader(
    std::istream& is,
...
class SelfClosingWriter {
 public:
  ...
  /**
   * Queue a start element which resides in no namespace at all.
   *
   * \p undeclare_namespace tells whether this is the outermost such element.
   * That one undeclares the default namespace of the enclosing document, and
   * the elements nested in it inherit that.
   *
   * Mind that the counterpart is the plain \ref StopElement: a stop element
   * carries no attribute, and the names are not prefixed, so there is nothing
   * in it which the namespace could change.
   */
  void StartElementInNoNamespace(
    std::string name,
