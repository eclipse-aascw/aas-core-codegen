...
bool Instance_0(
  ...
  const types::IInstance* that = (
    static_cast<const types::IInstance*>(value)
  );
  return verification::Each(
    that->text()
  );
}
...
bool Pointer_0(
  ...
  const types::IPointer* that = (
    static_cast<const types::IPointer*>(value)
  );
...
const std::vector<Check>& ChecksOf(Shape shape) {
  ...
  switch (shape) {
    ...
    case Shape::kInstance: {
      static const std::vector<Check> checks = {
        {
          &Instance_0,
          L"The text must not be empty"
        },
        {
          &Instance_1,
          L"The text must have more than two characters"
        }
      };
      return checks;
    }
    case Shape::kPointer: {
      static const std::vector<Check> checks = {
        {
          &Pointer_0,
          L"The text must have more than one character"
        }
      };
      return checks;
    }
    case Shape::kJsonObjectOfName: {
      static const std::vector<Check> checks = {
        {
          &JsonObjectOfName_0,
          L"The text must not be empty"
...
using listOf_jsonObjectOf_Name = std::vector<nlohmann::json>;
...
std::unique_ptr<IIterator> Over_listOf_Instance(
  ...
  return Each(value, &ThroughPointer<types::IInstance>, recursive);
}

std::unique_ptr<IIterator> Over_jsonObjectOf_Name(
  const nlohmann::json& value,
  bool
) {
  return Chain(
    One(&value, Shape::kJSONObject),
    EachKey(value, Shape::kName)
  );
}

std::unique_ptr<IIterator> Over_listOf_jsonObjectOf_Name(
  const listOf_jsonObjectOf_Name& value,
  bool recursive
) {
  return Each(value, &Over_jsonObjectOf_Name, recursive);
}

std::unique_ptr<IIterator> Over_listOf_ListOf(
  ...
  return Each(value, &ThroughPointer<types::IListOf>, recursive);
}

std::unique_ptr<IIterator> Over_tupleOf2_ListOf_str(
  ...
  return AtIndex(0, Over(*std::get<0>(value), recursive));
}

std::unique_ptr<IIterator> Over_Instance(
  const types::IInstance& that,
  bool
) {
  return One(&that, Shape::kInstance);
}

std::unique_ptr<IIterator> Over_Pointer(
  const types::IPointer& that,
  bool
) {
  return One(&that, Shape::kPointer);
}

std::unique_ptr<IIterator> Over_JsonObjectOfName(
  const types::IJsonObjectOfName& that,
  bool
) {
  return One(&that, Shape::kJsonObjectOfName);
}

std::unique_ptr<IIterator> Over_ListOf(
  const types::IListOf& that,
  bool
) {
  return One(&that, Shape::kListOf);
}
...
std::unique_ptr<IIterator> Over_Something(
  ...
  return Chain(
    ...
    InProperty(
      ...
      Over(*that.pointer(), recursive)
    ),
    InProperty(
      iteration::Property::kJsonObjectOfName,
      Over(*that.json_object_of_name(), recursive)
    ),
    InProperty(
      iteration::Property::kMappings,
      Over_listOf_jsonObjectOf_Name(that.mappings(), recursive)
    ),
    InProperty(
  ...
  switch (instance.model_type()) {
    case types::ModelType::kInstance:
      return Over_Instance(
        dynamic_cast<const types::IInstance&>(instance),
        recursive
      );
    case types::ModelType::kPointer:
      return Over_Pointer(
        dynamic_cast<const types::IPointer&>(instance),
        recursive
      );
    case types::ModelType::kJsonObjectOfName:
      return Over_JsonObjectOfName(
        dynamic_cast<const types::IJsonObjectOfName&>(instance),
        recursive
      );
    case types::ModelType::kListOf:
      return Over_ListOf(
        dynamic_cast<const types::IListOf&>(instance),
        recursive
      );
