...
const char* const kNamespace = "https://dummy.com";
...
> kNodeKindToHumanReadableString = {
  ...
  {NodeKind::StartInNoNamespace, "a start element in no namespace"},
  {NodeKind::StopInNoNamespace, "a stop element in no namespace"},
};
...
std::string NodeToHumanReadableString(
  ...
  switch (node.kind()) {
    ...
    case NodeKind::Start:
    case NodeKind::StartInNoNamespace: {
      ...
      return common::Concat(
        "a start node <",
        start_node.name,
        start_node.in_namespace ? ">" : "> in no namespace"
      );
    }

    case NodeKind::Stop:
    case NodeKind::StopInNoNamespace: {
      ...
      return common::Concat(
        "a stop node </",
        stop_node.name,
        stop_node.in_namespace ? ">" : "> in no namespace"
      );
    }
  ...
  switch (node.kind()) {
    ...
    case NodeKind::StartInNoNamespace: {
      ...
      return common::Concat(
        ...
        start_node.in_namespace ? L">" : L"> in no namespace"
      );
    }
    ...
    case NodeKind::StopInNoNamespace: {
      ...
      return common::Concat(
        ...
        stop_node.in_namespace ? L">" : L"> in no namespace"
      );
    }
...
bool IsStopNodeWithNameInNoNamespace(
  ...
  if (node.kind() != NodeKind::StopInNoNamespace) {
    return false;
  }
...
void XMLCALL OnStartElement(
  ...
  // NOTE (mristin):
  // Expat spells a namespaced element as `namespace|local`, and an element
  // in no namespace as the bare local name. We accept the both: the elements
  // of the document reside in \ref kNamespace, and the elements of
  // the XML-RPC subset in no namespace at all. Which of the two is expected
  // where is decided by the caller, as it depends on the grammar and not on
  // the document.
  size_t separator_i = name_str.find(kNamespaceSeparator);

  const bool in_namespace = separator_i != std::string::npos;

  if (separator_i == 0) {
    std::string message = common::Concat(
      "The namespace is empty in the start element <",
      name_str,
      ">"
    );
  ...
  if (
    in_namespace
    && name_str.compare(0, separator_i, kNamespace) != 0
  ) {
    std::string message = common::Concat(
      "We expected the XML namespace ",
      kNamespace,
      ", or no namespace at all, but we got the namespace ",
...
void XMLCALL OnStopElement(
  ...
  if (separator_i == 0) {
    std::string message = common::Concat(
      "The namespace is empty in the stop element </",
      name_str,
      ">"
    );
...
ReaderMergingText::ReaderMergingText(
  ...
  initialized_(false),
  reader_(is, additional_attributes, buffer_size),
  current_(common::make_unique<BofNode>()),
  error_(false),
  eof_(false) {
  // Intentionally empty.
...
void SelfClosingWriter::StartElementInNoNamespace(
  ...
  pending_start_wo_text_ = std::move(name);
  pending_attributes_ = undeclare_namespace ? " xmlns=\"\"" : "";

  // NOTE (mristin):
  // An element which resides in no namespace is never the outermost one of
  // a document which declares kNamespace, so it undeclares the default
  // namespace instead of taking the attributes of the root. We still drop
  // them here, so that they can not end up on any later element.
  root_attributes_ = "";
}
