...
using listOf_AbstractItem = std::vector<
  std::shared_ptr<types::IAbstractItem>
>;

using listOf_Simple = std::vector<
  std::shared_ptr<types::ISimple>
>;

std::unique_ptr<impl::IIterator> Over_listOf_AbstractItem(
  const listOf_AbstractItem& value,
  bool recursive
) {
  return Each(value, &OneThenOver<types::IAbstractItem>, recursive);
}

std::unique_ptr<impl::IIterator> Over_listOf_Simple(
  const listOf_Simple& value,
  bool recursive
) {
  return Each(value, &OneThenOver<types::ISimple>, recursive);
}
...
std::unique_ptr<impl::IIterator> Over_Something(
  ...
  return Chain(
    ...
    InProperty(
      Property::kSomeSimples,
      Over_listOf_Simple(that.some_simples(), recursive)
    )
  );
}
