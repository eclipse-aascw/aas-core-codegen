...
class OfItem : public impl::IVerificator {
 public:
  OfItem(
    const std::shared_ptr<types::IClass>& instance
  );
...
bool OfItem::Done() const {
  return true;
}

const Error& OfItem::Get() const {
  throw std::logic_error(
...
long OfItem::Index() const {
  return -1;
}

std::unique_ptr<impl::IVerificator> OfItem::Clone() const {
...
std::unique_ptr<impl::IVerificator> NewNonRecursiveVerificator(
  ...
  switch (instance->model_type()) {
    case types::ModelType::kItem:
      return common::make_unique<
        non_recursive_verificator::OfItem
      >(
        instance
      );
