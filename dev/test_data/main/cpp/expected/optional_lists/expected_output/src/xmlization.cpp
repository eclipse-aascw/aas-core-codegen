...
template <
  ...
  typename std::enable_if<
    std::is_base_of<T, types::IItem>::value
  >::type* = nullptr
>
std::pair<
...
> ItemFromSequence(
  ...
  if (reader.node().kind() == xml_common::NodeKind::Error) {
    throw std::logic_error(
      "Unexpected unhandled XML error in ItemFromSequence. "
      "ItemFromSequence expects no reader error at entry."
    );
  }
  #endif
  ...
  while (true) {
    ...
    auto it = properties::kMapOfItem.find(
      name
    );
    if (it == properties::kMapOfItem.end()) {
      return NoInstanceAndDeserializationErrorWithCause<
        std::shared_ptr<T>
      >(
        common::Concat(
    ...
    const properties::OfItem property(
      it->second
    );

    switch (property) {
      case properties::OfItem::kText: {
...
> SomethingFromSequence(
  ...
  while (true) {
    ...
    switch (property) {
      case properties::OfSomething::kOptionalItems: {
        if (the_optional_items.has_value()) {
          error = DuplicatePropertyError(name);
          break;
        }
      ...
      case properties::OfSomething::kOptionalTexts: {
        if (the_optional_texts.has_value()) {
          error = DuplicatePropertyError(name);
          break;
        }
      ...
      case properties::OfSomething::kOptionalValues: {
        if (the_optional_values.has_value()) {
          error = DuplicatePropertyError(name);
          break;
        }
 ...
 * \return error, if any
 */
common::optional<xml_common::SerializationError> SerializeItemAsSequence(
...
common::optional<xml_common::SerializationError> SerializeSomethingAsSequence(
  ...
  if (that.optional_items().has_value()) {
    error = SerializePropertyAsElement(
      ...
      *(that.optional_items()),
      writer,
      iteration::Property::kOptionalItems,
      [](
  ...
  if (that.optional_texts().has_value()) {
    error = SerializePropertyAsElement(
      "optionalTexts",
      *(that.optional_texts()),
      writer,
