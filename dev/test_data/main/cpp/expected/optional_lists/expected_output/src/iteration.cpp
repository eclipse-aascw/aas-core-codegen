...
std::wstring PropertyToWstring(
  ...
  switch (property) {
    case Property::kOptionalItems:
      return L"optional_items";
    case Property::kOptionalTexts:
      return L"optional_texts";
    case Property::kOptionalValues:
      return L"optional_values";
    case Property::kText:
      return L"text";
    default:
...
/**
 * This iterator is always done as IItem
 * references no other instances.
 */
class IteratorOverItem : public impl::IIterator {
 public:
  IteratorOverItem(
  ...
  const std::shared_ptr<types::IClass>& Get() const override {
    throw std::logic_error(
      "You want to get from an IteratorOverItem, "
      "but the iterator is always done as "
      "IItem references "
      "no other instances."
    );
  ...
  std::unique_ptr<impl::IIterator> Clone() const override {
    return common::make_unique<IteratorOverItem>(*this);
  }

  void PrependToPath(Path*) const override {
    throw std::logic_error(
      "You want to prepend to path from an IteratorOverItem, "
      "but the iterator is always done as "
      "IItem references "
      "no other instances."
    );
  ...
  ~IteratorOverItem() override = default;
};  // class IteratorOverItem
  ...
  while (true) {
    switch (state_) {
      case 0: {
        ...
        if (!(casted_->optional_items().has_value())) {
          state_ = 4;
          continue;
        }

        property_ = Property::kOptionalItems;

        cursor_ = 0;
      }

      case 1: {
        if (!(*cursor_ < casted_->optional_items()->size())) {
          state_ = 3;
          continue;
        }
        ...
        >& the_optional_items(
          *(casted_->optional_items())
        );
        const auto& item_value = the_optional_items[*cursor_];
...
std::unique_ptr<impl::IIterator> NewNonRecursiveIterator(
  ...
  switch (instance->model_type()) {
    case types::ModelType::kItem:
      return common::make_unique<IteratorOverItem>(
        instance
      );
