...
std::set<std::string> kPropertiesInItem = {
  "text"
};
...
> DeserializeItem(
  ...
  if (!additional_properties) {
    for (const auto& key_val : json.items()) {
      auto it(
        kPropertiesInItem.find(key_val.key())
      );
      if (it == kPropertiesInItem.end()) {
        std::wstring message = common::Concat(
...
> DeserializeSomething(
  ...
  // region De-serialize optionalItems

  if (json.contains("optionalItems")) {
    std::tie(
      the_optional_items,
      error
  ...
  // endregion De-serialize optionalItems

  // region De-serialize optionalTexts

  if (json.contains("optionalTexts")) {
    std::tie(
      the_optional_texts,
      error
  ...
  // endregion De-serialize optionalTexts
...
> SerializeSomething(
  ...
  const common::optional<
    std::vector<
      std::shared_ptr<types::IItem>
    >
  >& maybe_optional_items(
    that.optional_items()
  );
  if (that.optional_items().has_value()) {
    common::optional<nlohmann::json> json_optional_items;
    std::tie(
      json_optional_items,
      error
    ) = SerializeListWithFallible(
    ...
    result["optionalItems"] = std::move(
      json_optional_items.value()
    );
  }
  ...
  if (that.optional_texts().has_value()) {
    result["optionalTexts"] = SerializeListWithInfallible(
      *maybe_optional_texts,
      SerializeWstring
    );
  }
