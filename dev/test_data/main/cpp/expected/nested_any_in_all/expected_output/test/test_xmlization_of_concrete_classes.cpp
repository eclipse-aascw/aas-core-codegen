...
TEST_CASE("Test the round-trip of an expected LangString") {
...
TEST_CASE("Test the de-serialization failure on an unexpected LangString") {
  for (
...
TEST_CASE("Test the round-trip of an expected LangStringSet") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineXmlDir()
        / "Expected"
...
TEST_CASE("Test the de-serialization failure on an unexpected LangStringSet") {
  for (
    const std::filesystem::path& causeDir
    : test::common::ListSubdirectories(
      DetermineXmlDir()
...
TEST_CASE("Test the round-trip of an expected IecContent") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineXmlDir()
        / "Expected"
...
TEST_CASE("Test the de-serialization failure on an unexpected IecContent") {
  for (
    const std::filesystem::path& causeDir
    : test::common::ListSubdirectories(
      DetermineXmlDir()
...
TEST_CASE("Test the round-trip of an expected OtherContent") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineXmlDir()
        / "Expected"
...
TEST_CASE("Test the de-serialization failure on an unexpected OtherContent") {
  for (
    const std::filesystem::path& causeDir
    : test::common::ListSubdirectories(
      DetermineXmlDir()
...
TEST_CASE("Test the round-trip of an expected Specification") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineXmlDir()
        / "Expected"
...
TEST_CASE("Test the de-serialization failure on an unexpected Specification") {
  for (
    const std::filesystem::path& causeDir
    : test::common::ListSubdirectories(
      DetermineXmlDir()
...
TEST_CASE("Test the de-serialization failure on a duplicate property") {
  ...
  const std::size_t start(
    original.find("<language>")
  );

  std::string property;
  if (start != std::string::npos) {
    const std::size_t end(
      original.find("</language>", start)
    );
  ...
  const std::size_t insertion_index(
    original.rfind("</langString>")
  );

  INFO(aas::common::Concat("Looking for </langString> in ", path.string()))
