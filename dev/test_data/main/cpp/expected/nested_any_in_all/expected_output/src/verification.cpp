...
bool LangStringSetsHaveEnglish(
  const std::vector<
  ...
  return (
    common::All(
      [&](const std::shared_ptr<types::ILangStringSet>& lang_string_set) -> bool {
        return common::Some(
          [&](const std::shared_ptr<types::ILangString>& lang_string) -> bool {
            return IsEnglish(
              lang_string->language()
            );
          },
          lang_string_set->lang_strings()
        );
      },
      lang_string_sets
    )
...
bool IecContentsHaveDefinitionInEnglish(
  const std::vector<
  ...
  return (
    common::All(
      [&](const std::shared_ptr<types::ISpecification>& specification) -> bool {
        return !types::IsIecContent(*specification->content())
        || ((
          (std::dynamic_pointer_cast<types::IIecContent>(specification->content())->definition().has_value())
          && common::Some(
            [&](const std::shared_ptr<types::ILangString>& lang_string) -> bool {
              return IsEnglish(
                lang_string->language()
            ...
            (*(std::dynamic_pointer_cast<types::IIecContent>(specification->content())->definition()))
          )
        ));
      },
      specifications
...
bool Something_0(
  ...
  return !(that->specifications().has_value())
  || verification::IecContentsHaveDefinitionInEnglish(
    (*(that->specifications()))
  );
}
...
bool Something_1(
  ...
  return common::All(
    [&](const std::shared_ptr<types::ILangStringSet>& lang_string_set) -> bool {
      return common::Some(
        [&](const std::shared_ptr<types::ILangString>& lang_string) -> bool {
          return lang_string->language() == that->default_language();
        },
        lang_string_set->lang_strings()
      );
    },
...
bool Something_2(
  ...
  return verification::LangStringSetsHaveEnglish(
    that->lang_string_sets()
  );
}
  ...
  switch (shape) {
    case Shape::kSomething: {
      static const std::vector<Check> checks = {
        {
          &Something_0,
          L"The IEC contents must have a definition at least in English."
        },
        {
          &Something_1,
          L"Every language string set must have at least one string in "
          L"the default language."
        },
        {
          &Something_2,
          L"Every language string set must have at least one string in "
