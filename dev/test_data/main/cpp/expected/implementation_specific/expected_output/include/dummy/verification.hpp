...
/// \brief Check that the square brackets in \p text are balanced.
bool HasBalancedBrackets(
  const std::wstring& text
);

/// \brief Check that the \p texts do not repeat.
bool TextsAreUnique(
  const std::vector<std::wstring>& texts
);

/// \brief Check that the labels of the \p items do not repeat.
