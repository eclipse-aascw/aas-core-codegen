...
TEST_CASE("Test Descent over an Box") {
...
TEST_CASE("Test DescentOnce over an Box") {
...
TEST_CASE("Test Descent over an Bag") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineXmlDir()
        / "Expected"
...
TEST_CASE("Test DescentOnce over an Bag") {
  const std::shared_ptr<
    aas::types::IBag
  > instance(
    test::common::examples::LoadMaxBag()
  );
