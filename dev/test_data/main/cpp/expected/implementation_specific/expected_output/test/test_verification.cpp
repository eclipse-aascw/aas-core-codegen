...
TEST_CASE("Test verification of a valid Box") {
...
TEST_CASE("Test verification of invalid cases for Box") {
  for (
...
TEST_CASE("Test verification of a valid Bag") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineXmlDir()
        / "Expected"
...
TEST_CASE("Test verification of invalid cases for Bag") {
  for (
    const std::filesystem::path& causeDir
    : test::common::ListSubdirectories(
      DetermineXmlDir()
