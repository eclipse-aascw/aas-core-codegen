...
TEST_CASE("Test the round-trip of an expected Box") {
  ...
  for (const std::filesystem::path& path : paths) {
    AssertRoundTrip<
      aas::types::IBox
    >(path, aas::jsonization::BoxFrom);
  }
}

TEST_CASE("Test the de-serialization failure on an unexpected Box") {
  for (
...
TEST_CASE("Test the round-trip of an expected Bag") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineJsonDir()
        / "Expected"
  ...
  for (const std::filesystem::path& path : paths) {
    AssertRoundTrip<
      aas::types::IBag
    >(path, aas::jsonization::BagFrom);
  }
}

TEST_CASE("Test the de-serialization failure on an unexpected Bag") {
  for (
    const std::filesystem::path& causeDir
    : test::common::ListSubdirectories(
      DetermineJsonDir()
