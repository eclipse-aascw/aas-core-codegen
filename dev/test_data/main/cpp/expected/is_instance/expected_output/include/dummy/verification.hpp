...
/// \brief Check recursively whether there is a leaf in the tree of \p element.
///
...
/// \brief Check recursively that all the leaves in the tree of \p element
...
/// \brief Check that \p element is either not a leaf or has a short text.
///
...
/// \brief Check that \p element is a container.
///
...
/// \brief Check that \p element is an ordered container which is sorted.
///
...
/// \brief Check that \p container has at least one child.
bool ContainerHasChildren(
  const std::shared_ptr<types::IContainer>& container
);

/// \brief Check that \p value refers to a global attribute of kind name.
///
...
/// \brief Check that \p value is a string value.
///
