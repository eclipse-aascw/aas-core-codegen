...
std::wstring PropertyToWstring(
  ...
  switch (property) {
    ...
    case Property::kNumber:
      return L"number";
    case Property::kOptionalElement:
      return L"optional_element";
    case Property::kRoot:
      return L"root";
    case Property::kText:
      return L"text";
...
using listOf_Element = std::vector<
  std::shared_ptr<types::IElement>
>;

using listOf_Value = std::vector<types::Value>;

std::unique_ptr<impl::IIterator> Over_listOf_Element(
  const listOf_Element& value,
  bool recursive
) {
  return Each(value, &OneThenOver<types::IElement>, recursive);
}

std::unique_ptr<impl::IIterator> Over_AttributeItem(
  const types::AttributeItem& value,
  bool recursive
) {
  switch (value.index()) {
    case 0:
...
std::unique_ptr<impl::IIterator> Over_listOf_Value(
  const listOf_Value& value,
  bool recursive
) {
  return Each(value, &Over_Value, recursive);
}

std::unique_ptr<impl::IIterator> Over_OrderedContainer(
  const types::IOrderedContainer& that,
  bool recursive
) {
  return InProperty(
    Property::kChildren,
    Over_listOf_Element(that.children(), recursive)
  );
}

std::unique_ptr<impl::IIterator> Over_UnorderedContainer(
...
std::unique_ptr<impl::IIterator> Over_AttributeOperand(
  const types::IAttributeOperand& that,
  bool recursive
) {
  return InProperty(
    Property::kAttribute,
    Over_AttributeItem(that.attribute(), recursive)
  );
}
...
std::unique_ptr<impl::IIterator> Over_Something(
  ...
  return Chain(
    InProperty(
      Property::kRoot,
      OneThenOver(that.root(), recursive)
    ),
    InProperty(
      Property::kOptionalElement,
      that.optional_element().has_value()
        ? OneThenOver((*that.optional_element()), recursive)
        : Empty()
    ),
    InProperty(
      Property::kValue,
      Over_Value(that.value(), recursive)
    ),
    InProperty(
      Property::kValues,
      Over_listOf_Value(that.values(), recursive)
    )
  );
}
...
std::unique_ptr<impl::IIterator> DispatchOnModelType(
  ...
  switch (instance.model_type()) {
    case types::ModelType::kOrderedContainer:
      return Over_OrderedContainer(
        dynamic_cast<const types::IOrderedContainer&>(instance),
        recursive
      );
    case types::ModelType::kUnorderedContainer:
      return Over_UnorderedContainer(
        dynamic_cast<const types::IUnorderedContainer&>(instance),
        recursive
      );
    case types::ModelType::kAttributeOperand:
      return Over_AttributeOperand(
        dynamic_cast<const types::IAttributeOperand&>(instance),
        recursive
      );
