...
/// \brief Check the slice up to the position found with `find`.
bool DateBeforeTimeIsLongEnough(
  const std::wstring& text
);

/// \brief Check the slice from the position after the one found with `find`.
bool TimeAfterDateIsLongEnough(
  const std::wstring& text
);

/// \brief Check `find` with a start and the slice between the found positions.
bool MonthIsSeptember(
  const std::wstring& text
);

/// \brief Check the slices and `find` with negative literal positions.
bool SecondsFollowColon(
  const std::wstring& text
);

/// \brief Check the positions computed at run time which are negative or out of range.
///
...
/// \brief Check the slice of a constrained primitive with literal bounds.
bool NameStartsWithPrefix(
  const std::wstring& name
);

/// \brief Check `find` on a constrained primitive and a slice with a literal start.
