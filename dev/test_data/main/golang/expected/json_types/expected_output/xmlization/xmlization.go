...
func readSomethingAsSequence(
	...
	foundValues := false
	foundMapping := false
	foundMappingWithConstrainedKey := false
	foundOptionalValue := false
	foundOptionalValues := false
	...
	for {
		...
		case "value":
			...
			theValue, current, valueErr = readJsonValue(
				decoder, current,
			)
			foundValue = true
		case "values":
		...
		case "mapping":
			...
			theMapping, current, valueErr = readJsonObject(
				decoder, current,
			)
			foundMapping = true
		case "mappingWithConstrainedKey":
			if foundMappingWithConstrainedKey {
				valueErr = duplicatePropertyError(local)
				break
			}
			theMappingWithConstrainedKey, current, valueErr = readJsonObject(
				decoder, current,
			)
			foundMappingWithConstrainedKey = true
		case "optionalValue":
			if foundOptionalValue {
				valueErr = duplicatePropertyError(local)
				break
			}
			theOptionalValue, current, valueErr = readJsonValue(
				decoder, current,
			)
			foundOptionalValue = true
		case "optionalValues":
			if foundOptionalValues {
				valueErr = duplicatePropertyError(local)
				break
			}
			theOptionalValues, current, valueErr = readJsonArray(
				decoder, current,
			)
			foundOptionalValues = true
		case "optionalMapping":
			if foundOptionalMapping {
				valueErr = duplicatePropertyError(local)
				break
			}
			theOptionalMapping, current, valueErr = readJsonObject(
				decoder, current,
			)
			foundOptionalMapping = true
		default:
	...
	if !foundMapping {
		err = missingProperty("mapping")
		return
	}

	if !foundMappingWithConstrainedKey {
		err = missingProperty("mappingWithConstrainedKey")
		return
	}
	...
	instance.SetOptionalValue(theOptionalValue)
	instance.SetOptionalValues(theOptionalValues)
...
func writeSomethingAsSequence(
	...
	err = finishProperty(
		"Value()",
		writeElement(
			encoder, "value", that.Value(), writeJsonValue,
		),
	)
	if err != nil {
		return
	...
	err = finishProperty(
		"Values()",
		writeElement(
			encoder, "values", that.Values(), writeJsonArray,
		),
	)
	if err != nil {
		return
	...
	err = finishProperty(
		"Mapping()",
		writeElement(
			encoder, "mapping", that.Mapping(), writeJsonObject,
		),
	)
	if err != nil {
		return
	...
	err = finishProperty(
		"OptionalValue()",
		writeOptionalJsonValue(
			encoder, "optionalValue", that.OptionalValue(), writeJsonValue,
		),
	)
	if err != nil {
		return
	...
	err = finishProperty(
		"OptionalValues()",
		writeOptionalSlice(
			encoder, "optionalValues", that.OptionalValues(), writeJsonArray,
		),
	)
	if err != nil {
		return
	...
	err = finishProperty(
		"OptionalMapping()",
		writeOptionalJsonObject(
			encoder, "optionalMapping", that.OptionalMapping(), writeJsonObject,
		),
	)
	if err != nil {
		return
