...
func somethingFromMapWithoutDispatch(
	...
	for k, v := range m {
		switch k {
		case "someFloat":
			theSomeFloat, err = float64FromJsonable(v)
			foundSomeFloat = true

		case "someOptionalFloat":
			theSomeOptionalFloat, err = parseOptional(float64FromJsonable(v))
		...
		case "somePair":
			theSomePair, err = parseTuple2(v, stringFromJsonable, float64FromJsonable)
			foundSomePair = true
...
func somethingToMap(
	...
	result["someFloat"], err = float64ToJsonable(that.SomeFloat())
	if err != nil {
		mustSerializationError(err).prependName("SomeFloat()")
		return
	}

	if that.SomeOptionalFloat() != nil {
		result["someOptionalFloat"], err = float64ToJsonable(
			*(that.SomeOptionalFloat()),
		)
		if err != nil {
			mustSerializationError(err).prependName("SomeOptionalFloat()")
			return
		}
	}
	...
	result["somePair"], err = serializeTuple2(
		that.SomePair(), directToJsonable[string], float64ToJsonable,
	)
	if err != nil {
		mustSerializationError(err).prependName("SomePair()")
		return
	}
