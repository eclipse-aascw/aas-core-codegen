...
func somethingFromMapWithoutDispatch(
	...
	for k, v := range m {
		switch k {
		case "kind":
			theKind, err = KindFromJsonable(v)
			foundKind = true

		case "numbers":
			theNumbers, err = parseArray(v, int64FromJsonable)
			foundNumbers = true
...
func itemToMap(
	...
	result["texts"], err = serializeArray(that.Texts(), directToJsonable[string])
	if err != nil {
...
func somethingToMap(
	...
	result["kind"], err = KindToJsonable(that.Kind())
	if err != nil {
		mustSerializationError(err).prependName("Kind()")
		return
	}

	result["numbers"], err = serializeArray(that.Numbers(), int64ToJsonable)
	if err != nil {
