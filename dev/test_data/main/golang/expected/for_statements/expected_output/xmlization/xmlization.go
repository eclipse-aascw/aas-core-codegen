...
func readItemAsSequence(
	...
	for {
		var local string
		var ok bool
		local, current, ok, err = nextProperty(decoder, current, "IItem")
		if err != nil {
			return
		}
		if !ok {
		...
		case "texts":
			...
			theTexts, current, valueErr = readListOf(
...
func readItemDispatched(
	...
	default:
		err = unexpectedStartElement(local, "item")
	}
	return
}
...
func readSomethingAsSequence(
	...
	for {
		...
		case "numbers":
			...
			theNumbers, current, valueErr = readListOf(
...
func writeItemAsSequence(
	...
	err = finishProperty(
		...
		xmlcommon.WriteElement(
			encoder, "texts", that.Texts(), writeListOf_string,
		),
	)
	if err != nil {
		return
...
func writeSomethingAsSequence(
	...
	err = finishProperty(
		"Numbers()",
		xmlcommon.WriteElement(
			encoder, "numbers", that.Numbers(), writeListOf_long,
		),
	)
	if err != nil {
		return
