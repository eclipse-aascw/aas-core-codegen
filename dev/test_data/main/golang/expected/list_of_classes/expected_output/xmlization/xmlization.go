...
func readSimpleAsSequence(
	...
	for {
		var local string
		var ok bool
		local, current, ok, err = nextProperty(decoder, current, "ISimple")
		if err != nil {
			return
		}
		if !ok {
...
func readSimpleDispatched(
	...
	switch local {
	case "simple":
		instance, next, err = readSimpleAsSequence(decoder, current)
	default:
		err = unexpectedStartElement(local, "simple")
	}
	return
}
...
func readSomethingAsSequence(
	...
	for {
		...
		case "someItems":
			...
			theSomeItems, current, valueErr = readListOf(
				decoder, current, readAbstractItemDispatched,
			)
			foundSomeItems = true
		case "someSimples":
			if foundSomeSimples {
			...
			theSomeSimples, current, valueErr = readListOf(
				decoder, current, readSimpleDispatched,
			)
	...
	if !foundSomeSimples {
		err = missingProperty("someSimples")
		return
	}
