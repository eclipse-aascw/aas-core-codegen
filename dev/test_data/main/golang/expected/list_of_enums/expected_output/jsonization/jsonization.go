...
func somethingFromMapWithoutDispatch(
	...
	for k, v := range m {
		switch k {
		case "someResults":
			theSomeResults, err = parseArray(v, ResultFromJsonable)
...
func somethingToMap(
	...
	result["someResults"], err = serializeArray(
		that.SomeResults(), resultAsJsonableInterface,
	)
	if err != nil {
		mustSerializationError(err).prependName("SomeResults()")
		return
	}
