...
func somethingFromMapWithoutDispatch(
	...
	for k, v := range m {
		switch k {
		case "someBool":
			theSomeBool, err = boolFromJsonable(v)
			foundSomeBool = true

		case "someInt":
			theSomeInt, err = int64FromJsonable(v)
			foundSomeInt = true
		...
		case "someString":
			theSomeString, err = stringFromJsonable(v)
			foundSomeString = true

		case "someBytes":
			theSomeBytes, err = bytesFromJsonable(v)
			foundSomeBytes = true
...
func somethingToMap(
	...
	result["someInt"], err = int64ToJsonable(that.SomeInt())
	if err != nil {
		mustSerializationError(err).prependName("SomeInt()")
		return
	}
	...
	result["someBytes"], err = bytesToJsonable(that.SomeBytes())
	if err != nil {
