...
func readItemDispatched(
	...
	switch local {
	case "bag":
		instance, next, err = readBagAsSequence(decoder, current)
	case "box":
		instance, next, err = readBoxAsSequence(decoder, current)
	default:
		err = unexpectedDiscriminator(local, "IItem")
	}
	return
}
...
func readBoxAsSequence(
	...
	for {
		var local string
		var ok bool
		local, current, ok, err = nextProperty(decoder, current, "IBox")
		if err != nil {
			return
		}
		if !ok {
		...
		case "label":
			...
			theLabel, current, valueErr = xmlcommon.ReadText(
				decoder, current,
			)
			foundLabel = true
		case "color":
			...
			theColor, current, valueErr = readOptional(
				readTextAs_Color(decoder, current),
			)
			foundColor = true
		default:
...
func readBagAsSequence(
	...
	for {
		var local string
		var ok bool
		local, current, ok, err = nextProperty(decoder, current, "IBag")
		if err != nil {
			return
		}
		if !ok {
		...
		case "tags":
			...
			theTags, current, valueErr = readListOf(
...
func readContainerAsSequence(
	...
	for {
		...
		case "names":
			...
			theNames, current, valueErr = readListOf(
				decoder, current, readAtV_string,
			)
			foundNames = true
		case "items":
			...
			theItems, current, valueErr = readListOf(
...
func writeBoxAsSequence(
	...
	err = finishProperty(
		...
		xmlcommon.WriteElement(
			encoder, "label", that.Label(), xmlcommon.WriteAsText_string,
		),
	)
	if err != nil {
		return
	...
	err = finishProperty(
		...
		writeOptionalPointer(
			encoder, "color", that.Color(), writeAsText_Color,
		),
	)
	if err != nil {
		return
...
func writeBagAsSequence(
	...
	err = finishProperty(
		"Tags()",
		xmlcommon.WriteElement(
			encoder, "tags", that.Tags(), writeListOf_string,
		),
	)
	if err != nil {
		return
...
func writeContainerAsSequence(
	...
	err = finishProperty(
		"Names()",
		xmlcommon.WriteElement(
			encoder, "names", that.Names(), writeListOf_string,
		),
	)
	if err != nil {
		return
