...
def _list_of__result_from_jsonable(
    jsonable: Jsonable
...
def something_from_jsonable(
    ...
    the_some_results: Optional[List[aas_types.Result]] = None
    ...
    try:
        for key, jsonable_value in mapping.items():
            if key == 'modelType':
                ...
                pass
            elif key == 'someResults':
                the_some_results = _list_of__result_from_jsonable(jsonable_value)
            else:
                raise DeserializationException(
...
def _list_of__result_to_jsonable(
    that: List[aas_types.Result]
...
def _something_to_jsonable(
    ...
    try:
        jsonable['someResults'] = _list_of__result_to_jsonable(
            that.some_results
        )
    except SerializationException as exception:
        exception._prepend_property('some_results')
        raise
    return jsonable
