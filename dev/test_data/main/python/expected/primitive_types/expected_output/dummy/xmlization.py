...
def _read_something_as_sequence(
    ...
    the_some_bool: Optional[bool] = values.get('someBool')
    the_some_int: Optional[int] = values.get('someInt')
    the_some_float: Optional[float] = values.get('someFloat')
    the_some_string: Optional[str] = values.get('someString')
    the_some_bytes: Optional[bytes] = values.get('someBytes')
...
_READERS_FOR_SOMETHING: Mapping[
    ...
    'someBool': _read_bool_from_element_text,
    'someInt': _read_int_from_element_text,
    'someFloat': _read_float_from_element_text,
    'someString': read_str_from_element_text,
    'someBytes': _read_bytes_from_element_text,
}
...
def _write_something_as_element(
    ...
    try:
        serializer.writer.write_start_element(name)
        _write_bool_as_element('someBool', 'some_bool', that.some_bool, serializer)
        _write_int_as_element('someInt', 'some_int', that.some_int, serializer)
        _write_float_as_element('someFloat', 'some_float', that.some_float, serializer)
        _write_str_as_element('someString', 'some_string', that.some_string, serializer)
        _write_bytes_as_element('someBytes', 'some_bytes', that.some_bytes, serializer)
        serializer.writer.write_end_element(name)
    except Exception as exception:
        _attribute_to_property(exception, prop_name)
