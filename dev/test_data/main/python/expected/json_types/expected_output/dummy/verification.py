    ...
    def transform_something(
        ...
        for key in that.mapping_with_constrained_key:
            for error in verify_non_empty_string(key):
                ...
                # the message instead, as it does in ``_verify_json_value``.
                #
                # The error of a constrained primitive carries an empty path, so
                # nothing is lost by making a new one here.
                key_error = Error(f"In the member {key!r}: {error.cause}")
                key_error.path._prepend(
                    PropertySegment(
                        that,
                        'mapping_with_constrained_key'
                    )
