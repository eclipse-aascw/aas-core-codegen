...
def item_from_jsonable(
    ...
    :return: Concrete instance of :py:class:`.types.Item`
    :raise: :py:class:`DeserializationException` if unexpected :paramref:`jsonable`
    """
    return _dispatch_from_jsonable(
        jsonable,
        _ITEM_FROM_JSONABLE_DISPATCH,
        'Item'
    )
...
def box_from_jsonable(
    ...
    :param jsonable: structure to be parsed
    :return: Parsed instance of :py:class:`.types.Box`
    :raise: :py:class:`DeserializationException` if unexpected :paramref:`jsonable`
    """
    mapping = _as_mapping(jsonable)
    ...
    if model_type != 'Box':
        raise DeserializationException(
            f"Expected modelType to be 'Box', "
    ...
    try:
        for key, jsonable_value in mapping.items():
            if key == 'modelType':
                ...
                the_label = _str_from_jsonable(jsonable_value)
            elif key == 'color':
                the_color = color_from_jsonable(jsonable_value)
            else:
                raise DeserializationException(
...
def bag_from_jsonable(
    ...
    :param jsonable: structure to be parsed
    :return: Parsed instance of :py:class:`.types.Bag`
    :raise: :py:class:`DeserializationException` if unexpected :paramref:`jsonable`
    """
    mapping = _as_mapping(jsonable)
    ...
    if model_type != 'Bag':
        raise DeserializationException(
            f"Expected modelType to be 'Bag', "
            f"but got: {model_type!r}"
        )

    the_label: Optional[str] = None
    the_tags: Optional[List[str]] = None
    ...
    try:
        for key, jsonable_value in mapping.items():
            if key == 'modelType':
                ...
                the_tags = _list_of__str_from_jsonable(jsonable_value)
            else:
                raise DeserializationException(
                    f"Unexpected property: {key}"
                )
    ...
    try:
        for key, jsonable_value in mapping.items():
            if key == 'modelType':
                ...
                the_names = _list_of__str_from_jsonable(jsonable_value)
            elif key == 'items':
...
def _box_to_jsonable(
    ...
    try:
        jsonable['label'] = that.label
    except SerializationException as exception:
        exception._prepend_property('label')
        raise
    if that.color is not None:
        try:
            jsonable['color'] = that.color.value
        except SerializationException as exception:
            exception._prepend_property('color')
            raise
...
def _bag_to_jsonable(
    ...
    try:
        ...
        exception._prepend_property('tags')
        raise
...
def _container_to_jsonable(
    ...
    try:
        ...
        exception._prepend_property('names')
        raise
    try:
        jsonable['items'] = _list_of__item_to_jsonable(
            that.items
        )
    except SerializationException as exception:
...
class _Serializer(
    ...
    transform_box = staticmethod(
        _box_to_jsonable
    )
    transform_bag = staticmethod(
        _bag_to_jsonable
    )
