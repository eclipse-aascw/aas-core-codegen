...
class TestRoundTrips(unittest.TestCase):
    def test_item(self) -> None:
        for path in sorted(
            ...
            got_from_iterparse = (
                aas_xmlization.item_from_iterparse(iterator)
            )
...
class TestDuplicateProperty(unittest.TestCase):
    ...
    def test_duplicate_property(self) -> None:
        ...
        start = text.find('<texts>')
        if start >= 0:
            end = text.find('</texts>', start)
            duplicated = text[start:end + 8]
        else:
        ...
        with self.assertRaises(aas_xmlization.DeserializationException):
            aas_xmlization.item_from_str(broken_text)
