...
def query_condition_from_iterparse(
    ...
    return _read_instance_from_iterparse(
        iterator,
        _read_query_condition_as_element,
        'QueryCondition'
    )
...
def _read_query_condition_as_sequence(
    ...
    values = _read_properties(
        element,
        iterator,
        _READERS_FOR_QUERY_CONDITION
    )

    the_eq: Optional[str] = values.get('eq')
    the_not_eq: Optional[str] = values.get('not-eq')
...
def _read_query_condition_as_element(
    ...
    return _read_named_element(
        ...
        iterator,
        'queryCondition',
        _read_query_condition_as_sequence
    )
...
#: :py:class:`.types.QueryCondition`, by the XML name of the property
_READERS_FOR_QUERY_CONDITION: Mapping[
    str,
    _ContentReader[Any]
] = {
    'eq': _read_str_from_element_text,
    'not-eq': _read_str_from_element_text,
}
...
def _write_query_condition_as_element(
    name: str,
    prop_name: Optional[str],
    ...
    try:
        ...
        if that.eq is not None:
            _write_str_as_element('eq', 'eq', that.eq, serializer)
        if that.not_eq is not None:
            _write_str_as_element('not-eq', 'not_eq', that.not_eq, serializer)
        serializer._write_end_element(name)
    except Exception as exception:
        _attribute_to_property(exception, prop_name)
...
class _Serializer(aas_types.AbstractVisitor):
    ...
    def visit_query_condition(
        ...
        :param that: instance to be serialized
        """
        _write_query_condition_as_element('queryCondition', None, that, self)
