...
test("Leaf deserialization fails on non-object", () => {
...
test("Leaf deserialization fails with missing required property", () => {
...
test("Leaf deserialization fails with property type mismatch", () => {
  const jsonable = readExpectedJson(
    "Leaf",
    "minimal.json"
  );
...
test("Something deserialization fails with invalid nested modelType", () => {
  ...
  const jsonObject = { ...mustBeJsonObject(jsonable) };
  const nestedValue = jsonObject["root"];
