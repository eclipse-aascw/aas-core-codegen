...
test("XML namespace mismatch fails", () => {
  const pth = path.join(
    TestCommon.TEST_DATA_DIR, "Xml", "Expected", "leaf", "minimal.xml"
  );
...
test("XML duplicate property fails", () => {
  ...
  if (root.tagName !== "leaf") {
    throw new Error(
      `Expected root element <leaf>, ` +
      `got <${root.tagName}>`
    );
  }
