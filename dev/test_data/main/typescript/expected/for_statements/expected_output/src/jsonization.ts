 ...
 */
export function kindFromJsonable(
  jsonable: JsonValue
): AasCommon.Either<AasTypes.Kind, DeserializationError> {
  if (typeof jsonable !== "string") {
    return newDeserializationError<AasTypes.Kind>(
      `Expected a string, but got: ${typeof jsonable}`
    );
  }

  const literal = AasStringification.kindFromString(jsonable);
  if (literal === null) {
    return newDeserializationError<AasTypes.Kind>(
      "Not a valid string representation of " +
        `a literal of Kind: ${jsonable}`
    );
  }
 ...
 * of {@link types!Item} from `jsonObject`.
 *
 * @param jsonObject - JSON object to be parsed
 * @returns parsed instance of {@link types!Item},
 * or an error if any
 */
function parsePropertiesOfItem(
  jsonObject: JsonObject
): AasCommon.Either<
  AasTypes.Item,
  DeserializationError
...
/**
 * Parse an instance of {@link types!Item} from the JSON-able
 * structure `jsonable`.
 *
...
> {
  ...
  return parsePropertiesOfItem(jsonObject);
}
...
function parsePropertiesOfSomething(
  ...
  DeserializationError
> {
  let theKind: AasTypes.Kind | null = null;
  let theNumbers: Array<number> | null = null;
  let theTexts: Array<string> | null = null;
 ...
 * {@link types!Kind}
 */
function serialize_Kind(
  that: AasTypes.Kind
): string {
  const text = AasStringification.kindToString(that);
  if (text === null) {
    throw new SerializationError(
      `Invalid literal of Kind: ${that}`
    );
  }
  return text;
}
...
function serializeSomething(
  ...
  try {
    prop = "kind";
    jsonable["kind"] =
      serialize_Kind(that.kind);

    prop = "numbers";
    jsonable["numbers"] =
      serialize_ListOf_int(that.numbers);
...
function serialize_ListOf_int(
  ...
  try {
    for (; i < that.length; i++) {
      result[i] = integerToJsonable(that[i]);
    }
  } catch (error) {
    if (error instanceof SerializationError) {
 ...
 * @returns JSON-able array
 */
function serialize_ListOf_Item(
  that: ReadonlyArray<AasTypes.Item>
): Array<JsonObject> {
  const result = new Array<JsonObject>(that.length);
  let i = 0;
  try {
    for (; i < that.length; i++) {
      result[i] = serializeItem(that[i]);
    }
  } catch (error) {
    if (error instanceof SerializationError) {
      error.prependIndex(i);
