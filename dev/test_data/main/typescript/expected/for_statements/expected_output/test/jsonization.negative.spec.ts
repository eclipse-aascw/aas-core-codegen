...
test("Item deserialization fails on non-object", () => {
...
test("Item deserialization fails with missing required property", () => {
...
test("Item deserialization fails with property type mismatch", () => {
  const jsonable = readExpectedJson(
    "Item",
    "minimal.json"
  );
  ...
  const jsonObject = { ...mustBeJsonObject(jsonable) };
  jsonObject["texts"] = { definitely: "unexpected-object" };
