...
test("LangString deserialization fails with missing required property", () => {
...
test("LangString deserialization fails with property type mismatch", () => {
  const jsonable = readExpectedJson(
    "LangString",
    "minimal.json"
  );
...
test("Specification deserialization fails with invalid nested modelType", () => {
  const jsonable = readExpectedJson(
    "Specification",
    "maximal.json"
  );
  ...
  const jsonObject = { ...mustBeJsonObject(jsonable) };
  const nestedValue = jsonObject["content"];
