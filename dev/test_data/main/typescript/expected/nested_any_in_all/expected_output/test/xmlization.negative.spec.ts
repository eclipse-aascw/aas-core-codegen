...
test("XML namespace mismatch fails", () => {
  const pth = path.join(
    TestCommon.TEST_DATA_DIR, "Xml", "Expected", "langString", "minimal.xml"
  );
...
test("XML duplicate property fails", () => {
  ...
  if (root.tagName !== "langString") {
    throw new Error(
      `Expected root element <langString>, ` +
      `got <${root.tagName}>`
    );
  }
...
test("XML nested class dispatch mismatch fails", () => {
  ...
  const pth = path.join(
    TestCommon.TEST_DATA_DIR, "Xml", "Expected", "specification", "maximal.xml"
  );
  const [document, parseError] = parseXml(fs.readFileSync(pth, "utf-8"));
  if (parseError !== null) {
    throw new Error(`Invalid XML fixture: ${parseError}`);
  ...
  const root = document.documentElement;
  if (root.tagName !== "specification") {
    throw new Error(
      `Expected root element <specification>, ` +
      `got <${root.tagName}>`
    );
  }
