...
> {
  ...
  return parsePropertiesOfLangString(jsonObject);
}

/**
 * Parse the properties of an instance
 * of {@link types!LangStringSet} from `jsonObject`.
 *
 * @param jsonObject - JSON object to be parsed
 * @returns parsed instance of {@link types!LangStringSet},
 * or an error if any
 */
function parsePropertiesOfLangStringSet(
  jsonObject: JsonObject
): AasCommon.Either<
...
/**
 * Parse an instance of {@link types!LangStringSet} from the JSON-able
 * structure `jsonable`.
 *
 * @param jsonable - structure to be parsed
 * @returns parsed instance of {@link types!LangStringSet},
 * or an error if any
 */
export function langStringSetFromJsonable(
  jsonable: JsonValue
): AasCommon.Either<
  AasTypes.LangStringSet,
  DeserializationError
> {
  ...
  return parsePropertiesOfLangStringSet(jsonObject);
}
 ...
 */
export function contentFromJsonable(
  jsonable: JsonValue
): AasCommon.Either<
  AasTypes.IContent,
  DeserializationError
> {
  ...
  switch (modelType) {
    case "IecContent":
      return parsePropertiesOfIecContent(jsonObject);

    case "OtherContent":
      return parsePropertiesOfOtherContent(jsonObject);
...
/**
 * Parse the properties of an instance
 * of {@link types!IecContent} from `jsonObject`.
 *
 * The `modelType` is expected to have been already verified by the caller,
 * and is therefore skipped here.
 *
 * @param jsonObject - JSON object to be parsed
 * @returns parsed instance of {@link types!IecContent},
 * or an error if any
 */
function parsePropertiesOfIecContent(
  jsonObject: JsonObject
): AasCommon.Either<
  AasTypes.IecContent,
  DeserializationError
...
/**
 * Parse an instance of {@link types!IecContent} from the JSON-able
 * structure `jsonable`.
 *
 * @param jsonable - structure to be parsed
 * @returns parsed instance of {@link types!IecContent},
 * or an error if any
 */
export function iecContentFromJsonable(
  jsonable: JsonValue
): AasCommon.Either<
  AasTypes.IecContent,
  DeserializationError
> {
  ...
  const modelTypeError = checkModelType(jsonObject, "IecContent");
...
/**
 * Parse the properties of an instance
 * of {@link types!OtherContent} from `jsonObject`.
 *
 * The `modelType` is expected to have been already verified by the caller,
 * and is therefore skipped here.
 *
 * @param jsonObject - JSON object to be parsed
 * @returns parsed instance of {@link types!OtherContent},
 * or an error if any
 */
function parsePropertiesOfOtherContent(
  jsonObject: JsonObject
): AasCommon.Either<
...
> {
  return new AasCommon.Either<
    ...
    DeserializationError
  >(
    new AasTypes.OtherContent(),
    null
  );
}

/**
 * Parse an instance of {@link types!OtherContent} from the JSON-able
 * structure `jsonable`.
 *
 * @param jsonable - structure to be parsed
 * @returns parsed instance of {@link types!OtherContent},
 * or an error if any
 */
export function otherContentFromJsonable(
  jsonable: JsonValue
): AasCommon.Either<
  AasTypes.OtherContent,
  DeserializationError
> {
  ...
  const modelTypeError = checkModelType(jsonObject, "OtherContent");
  if (modelTypeError !== null) {
    return new AasCommon.Either<
      AasTypes.OtherContent,
      DeserializationError
...
/**
 * Parse the properties of an instance
 * of {@link types!Specification} from `jsonObject`.
 *
 * @param jsonObject - JSON object to be parsed
 * @returns parsed instance of {@link types!Specification},
 * or an error if any
 */
function parsePropertiesOfSpecification(
  jsonObject: JsonObject
): AasCommon.Either<
...
> {
  ...
  for (const key in jsonObject) {
    ...
    switch (key) {
      case "content": {
        const parsed = contentFromJsonable(
          jsonableValue
        );
        propertyError = parsed.error;
...
/**
 * Parse an instance of {@link types!Specification} from the JSON-able
 * structure `jsonable`.
 *
 * @param jsonable - structure to be parsed
 * @returns parsed instance of {@link types!Specification},
 * or an error if any
 */
export function specificationFromJsonable(
  jsonable: JsonValue
): AasCommon.Either<
  AasTypes.Specification,
  DeserializationError
> {
  ...
  return parsePropertiesOfSpecification(jsonObject);
}
 ...
 * @returns JSON-able representation
 */
function serializeLangStringSet(
  ...
  try {
    prop = "langStrings";
    jsonable["langStrings"] =
      serialize_ListOf_LangString(that.langStrings);
  } catch (error) {
    if (error instanceof SerializationError) {
      error.prependProperty(prop);
    }
...
function serializeIecContent(
  ...
  try {
    if (that.definition !== null) {
      prop = "definition";
      jsonable["definition"] =
        serialize_ListOf_LangString(that.definition);
    }
  } catch (error) {
    if (error instanceof SerializationError) {
      error.prependProperty(prop);
 ...
 * @returns JSON-able representation
 */
function serializeOtherContent(
 ...
 * @returns JSON-able representation
 */
function serializeSpecification(
...
function serializeSomething(
  ...
  try {
    ...
    jsonable["defaultLanguage"] =
      that.defaultLanguage;

    prop = "langStringSets";
    jsonable["langStringSets"] =
      serialize_ListOf_LangStringSet(that.langStringSets);

    if (that.specifications !== null) {
      prop = "specifications";
      jsonable["specifications"] =
        serialize_ListOf_Specification(that.specifications);
    }
  } catch (error) {
    if (error instanceof SerializationError) {
      error.prependProperty(prop);
 ...
 */
function serialize_ListOf_LangString(
  that: ReadonlyArray<AasTypes.LangString>
): Array<JsonObject> {
  const result = new Array<JsonObject>(that.length);
  let i = 0;
  try {
    for (; i < that.length; i++) {
      result[i] = serializeLangString(that[i]);
    }
  } catch (error) {
    if (error instanceof SerializationError) {
 ...
 * @returns JSON-able array
 */
function serialize_ListOf_LangStringSet(
  that: ReadonlyArray<AasTypes.LangStringSet>
): Array<JsonObject> {
  const result = new Array<JsonObject>(that.length);
  let i = 0;
  try {
    for (; i < that.length; i++) {
      result[i] = serializeLangStringSet(that[i]);
    }
  } catch (error) {
    if (error instanceof SerializationError) {
      error.prependIndex(i);
 ...
 * @returns JSON-able array
 */
function serialize_ListOf_Specification(
  that: ReadonlyArray<AasTypes.Specification>
): Array<JsonObject> {
  const result = new Array<JsonObject>(that.length);
  let i = 0;
  try {
    for (; i < that.length; i++) {
      result[i] = serializeSpecification(that[i]);
    }
  } catch (error) {
    if (error instanceof SerializationError) {
      error.prependIndex(i);
...
class Serializer extends AasTypes.AbstractTransformer<JsonObject> {
  transformLangString(
    ...
    return serializeLangString(that);
  }

  transformLangStringSet(
    that: AasTypes.LangStringSet
  ): JsonObject {
    return serializeLangStringSet(that);
  }

  transformIecContent(
    that: AasTypes.IecContent
  ): JsonObject {
    return serializeIecContent(that);
  }

  transformOtherContent(
    that: AasTypes.OtherContent
  ): JsonObject {
    return serializeOtherContent(that);
  }

  transformSpecification(
    that: AasTypes.Specification
  ): JsonObject {
    return serializeSpecification(that);
  }
