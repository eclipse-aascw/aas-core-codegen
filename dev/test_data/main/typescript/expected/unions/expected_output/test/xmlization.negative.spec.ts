...
test("XML namespace mismatch fails", () => {
  const pth = path.join(
    TestCommon.TEST_DATA_DIR, "Xml", "Expected", "structuralFirst", "minimal.xml"
  );
...
test("XML duplicate property fails", () => {
  const propertyName = "uniqueToFirst";
  ...
  if (root.tagName !== "structuralFirst") {
    throw new Error(
      `Expected root element <structuralFirst>, ` +
      `got <${root.tagName}>`
    );
  }
...
test("XML list item element name mismatch fails", () => {
  const propertyName = "listStructuralProperty";
