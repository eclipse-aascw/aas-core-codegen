...
function jsonValueFromJsonable(
  ...
  if (jsonable === null || jsonable === undefined) {
    return newDeserializationError<AasTypes.JsonValue>(
      `Expected a JSON-able value, but got: ${jsonable}`
    );
  }
  ...
  switch (typeof jsonable) {
    ...
    case "number":
      ...
      if (!Number.isFinite(jsonable)) {
        return newDeserializationError<AasTypes.JsonValue>(
          `Expected a JSON-able value, but got the number ${jsonable}, ` +
    ...
    default:
      return newDeserializationError<AasTypes.JsonValue>(
        `Expected a JSON-able value (a boolean, a number, a string, ` +
          `an array or an object), but got: ${typeof jsonable}`
      );
  }
  ...
  if (Array.isArray(jsonable)) {
    ...
    for (let i = 0; i < jsonable.length; i++) {
      const parsed = jsonValueFromJsonable(jsonable[i]);
      if (parsed.error !== null) {
        parsed.error.path.prepend(new IndexSegment(jsonable, i));
        return parsed;
      }
  ...
  for (const key of Object.keys(jsonable)) {
    const parsed = jsonValueFromJsonable((jsonable as JsonObject)[key]);
    if (parsed.error !== null) {
      parsed.error.path.prepend(
        new KeySegment(jsonable as JsonObject, key)
      );
      return parsed;
    }
