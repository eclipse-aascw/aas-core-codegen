...
function parse_jsonValue(
  ...
  return parseElementContent(
    cursor, localName, parseXmlRpcDiscriminator(localName)
  );
}
...
function parseXmlRpcDiscriminator(
  ...
  switch (localName) {
    case "boolean":
      return parseXmlRpcBoolean;
    case "double":
      return parseXmlRpcDouble;
    case "string":
      return parseXmlRpcString;
    case "array":
      return parse_jsonArray;
    case "struct":
      return parse_jsonObject;
    default:
      return () =>
        newDeserializationError<AasTypes.JsonValue>(
...
function parseXmlRpcDouble(
  ...
  // NOTE (mristin):
  // See the note in `parseXmlRpcBoolean` on why the whitespace is
  // collapsed here.
  const text = collapseWhitespace(parseTextContent(cursor));
...
function parse_jsonArray(
  ...
  return parseNamedElement(cursor, "data", (dataCursor) => {
    const items = new Array<AasTypes.JsonValue>();

    // eslint-disable-next-line no-constant-condition
    while (true) {
      dataCursor.skipIgnorable();
      const token = dataCursor.current();
      if (token === null || !(token instanceof OpenTagToken)) {
        break;
      }

      const itemOrError = parseNamedElement(
        dataCursor, "value", parse_jsonValue
      );
      if (itemOrError.error !== null) {
        return new AasCommon.Either<AasTypes.JsonArray, DeserializationError>(
          null,
          new DeserializationError(
            `At the index ${items.length}: ${itemOrError.error.message}`,
            itemOrError.error.path
          )
        );
      }
...
function parse_jsonObject(
  ...
  while (true) {
    ...
    const memberOrError = parseNamedElement(
      cursor, "member", parseXmlRpcMember
    );
...
function parseXmlRpcMember(
  ...
  const keyOrError = parseNamedElement(cursor, "name", (nameCursor) =>
    new AasCommon.Either<string, DeserializationError>(
      parseTextContent(nameCursor), null
    )
  );
  ...
  const valueOrError = parseNamedElement(cursor, "value", parse_jsonValue);
  if (valueOrError.error !== null) {
    ...
    >(
      null,
      new DeserializationError(
        `In the member '${key}': ${valueOrError.error.message}`,
        valueOrError.error.path
      )
    );
  }
...
function write_jsonValue(
  ...
  switch (typeof value) {
    case "boolean":
      ...
      // use.
      parts.push(value ? "<boolean>1</boolean>" : "<boolean>0</boolean>");
      return;

    case "number":
      // NOTE (mristin):
      // JSON knows neither an infinity nor a not-a-number, so neither is
      // a JSON-able value, and `parseXmlRpcDouble` refuses to read either
      // back.
      if (!Number.isFinite(value)) {
        throw new SerializationError(
      ...
      parts.push(`<double>${value}</double>`);
      return;

    case "string":
      parts.push(`<string>${escapeXmlText(value)}</string>`);
      return;
  ...
  if (Array.isArray(value)) {
    parts.push("<array>");
    write_jsonArray(parts, value);
    parts.push("</array>");
    return;
  }

  parts.push("<struct>");
  write_jsonObject(parts, value as AasTypes.JsonObject);
  parts.push("</struct>");
}
...
function write_jsonArray(
  ...
  for (let i = 0; i < value.length; i++) {
    parts.push("<value>");
    try {
      write_jsonValue(parts, value[i]);
    } catch (error) {
      if (error instanceof SerializationError) {
        error.path.prepend(new IndexSegment(i));
      }
      throw error;
    }
    parts.push("</value>");
...
function write_jsonObject(
  ...
  for (const key of Object.keys(value)) {
    ...
    parts.push("<value>");
    try {
      write_jsonValue(parts, value[key]);
    } catch (error) {
      if (error instanceof SerializationError) {
        // NOTE (mristin):
        // A member of an open JSON object is no property of one of our
        // classes, so it gets a name segment of the JSON key, which is
        // the closest the path vocabulary has to offer.
        error.path.prepend(new NameSegment(key));
      }
      throw error;
    }
    parts.push("</value>");
