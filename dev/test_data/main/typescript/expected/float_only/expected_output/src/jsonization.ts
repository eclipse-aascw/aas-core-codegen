...
> {
  ...
  let theSomeOptionalFloat: number | null = null;
  let theSomeFloats: Array<number> | null = null;
  let theSomePair: [string, number] | null = null;
  ...
  for (const key in jsonObject) {
    ...
    switch (key) {
      ...
      case "someOptionalFloat": {
        ...
        propertyError = parsed.error;
        theSomeOptionalFloat = parsed.value;
        break;
      }
...
function serializeSomething(
  ...
  try {
    ...
    if (that.someOptionalFloat !== null) {
      prop = "someOptionalFloat";
      jsonable["someOptionalFloat"] =
        numberToJsonable(that.someOptionalFloat);
    }

    prop = "someFloats";
    jsonable["someFloats"] =
      serialize_ListOf_float(that.someFloats);

    prop = "somePair";
    jsonable["somePair"] =
      serialize_TupleOf2_str_float(that.somePair);
  } catch (error) {
 ...
 */
function serialize_ListOf_float(
  that: ReadonlyArray<number>
): Array<number> {
  const result = new Array<number>(that.length);
  let i = 0;
  try {
    for (; i < that.length; i++) {
      result[i] = numberToJsonable(that[i]);
    }
  } catch (error) {
    if (error instanceof SerializationError) {
 ...
 * @returns JSON-able array
 */
function serialize_TupleOf2_str_float(
  ...
  try {
    result[1] = numberToJsonable(that[1]);
  } catch (error) {
    if (error instanceof SerializationError) {
