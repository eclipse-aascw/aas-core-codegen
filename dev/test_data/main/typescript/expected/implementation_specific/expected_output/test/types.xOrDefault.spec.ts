...
test("Box.colorOrDefault on maximal", () => {
  const instance = TestCommonJsonization.loadMaximalBox();

  const value = new EnumerationLiteral(
    "Color",
    AasStringification.mustColorToString(
  ...
  compareOrRecordValue(
    ...
    path.join(
      ...
      "xOrDefault",
      "Box",
      "colorOrDefault.on_maximal.json"
    )
  );
});

test("Box.colorOrDefault on minimal", () => {
  ...
  compareOrRecordValue(
    ...
    path.join(
      ...
      "xOrDefault",
      "Box",
      "colorOrDefault.on_minimal.json"
    )
  );
});
