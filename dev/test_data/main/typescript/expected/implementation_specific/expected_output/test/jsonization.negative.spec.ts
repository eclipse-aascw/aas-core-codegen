...
test("Box deserialization fails on non-object", () => {
...
test("Box deserialization fails with missing required property", () => {
...
test("Box deserialization fails with property type mismatch", () => {
  const jsonable = readExpectedJson(
    "Box",
    "minimal.json"
  );
  ...
  const jsonObject = { ...mustBeJsonObject(jsonable) };
  jsonObject["label"] = { definitely: "unexpected-object" };
